...
if ($_SERVER[REMOTE_ADDR]=="1.217.39.202"){
	$testmode = 1;
}

if ((in_array($cfg[skin], $r_printhome_skin)) && $_GET[code]=="top_menu_banner")
{
  echo "<script>location.href='set.banner.menu.php?code=$_GET[code]';</script>";  
  exit;
}

if ($_COOKIE[skin]) $cfg[skin] = $_COOKIE[skin];
...
if ($cfg[skin]=="bluepod"){
	if ($_GET[code]=="main_index"){
		$img_w = "535";
		$img_h = "288";
	}else if ($_GET[code]=="left_banner"){
		$img_w = "228";
		$img_h = "256";
	}else if ($_GET[code]=="right_banner"){
		$img_w = "228";
		$img_h = "256";
	}
}
if ($cfg[skin]=="rose"){
	if ($_GET[code]=="main_index"){
		...
		$img_w = "210";
		$img_h = "105";
	}else if ($_GET[code]=="best_banner1"){
		$img_w = "248";
		$img_h = "238";
	}else if ($_GET[code]=="best_banner2"){
		$img_w = "248";
		$img_h = "238";
	}else if ($_GET[code]=="best_banner3"){
		$img_w = "248";
		$img_h = "238";
	}
}
	...
	<div id="content" class="content">
		<div id="header" class="header navbar navbar-default navbar-fixed-top">
            ...
            <div class="navbar-header">
               <a href="javascript:window.close();" class="navbar-brand"><span class="navbar-logo"></span><?=_("배너 관리")?></a>
         	</div>
      	</div>
   	</div>      
		<div class="row">
			<div class="col-md-12">
				<ul class="nav nav-tabs nav-tabs-inverse">
					<li class=""><a href="#tab-1" data-toggle="tab"><?=_("기본 이미지 배너")?></a></li>
					<li class=""><a href="#tab-2" data-toggle="tab"><?=_("텍스트 배너")?></a></li>
					<li class="active"><a href="#tab-3" data-toggle="tab"><?=_("에디터 배너")?></a></li>
				</ul>
				<div class="tab-content">
					<div class="tab-pane fade" id="tab-1">
						...
						<tr>
							<th><?=_("슬라이드속도")?></th>
							<td><input type="text" name="slide_speed" value="<?=$data[slide_speed]?>" size="1"><br><?=_("슬라이드 배너 일 경우 (초단위 입력 미입력 혹은 0일 경우 5초로 적용됩니다. basic 스킨에서만 적용이 됩니다.)")?></td>
						</tr>
						<tr>
							<th><p><?=_("배너이미지")?></p>
						  
						  <? 
						    if ($_GET[code] != 'top_menu_banner') { ?>	  
...
var oEditors = [];
smartEditorInit("edit_spc_desc", true, "editor", true);
