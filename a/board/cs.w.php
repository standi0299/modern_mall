...
$file_list = $m_board->getMycsFileInfo($_GET[no]);
         ...
         <div class="panel panel-inverse">
            ...
            <form class="form-horizontal form-bordered" name="fm" method="POST" action="indb.php" enctype="multipart/form-data">
               ...
               <div class="form-group">
                  ...
                  <div class="col-md-10">
                     <?=nl2br(stripslashes($data[content]))?>
                  </div>
               </div>
                  	 ...
                  	 <? if ($data[img]) { ?>
                  	 	<? if (count($file_list) > 0) { ?>
                     		<? foreach ($file_list as $key=>$val) {
                     			$filepath = "../../data/board/$cid/cs/$val[filesrc]";
                     			$size = getimagesize($filepath);
                     			
                     			if (is_array($size)) {
                     				if ($size[0] > 800) $data[img_w] = "800px";
                     			?>
                     				<img src="<?=$filepath?>" style="display:block;margin-bottom:5px;<? if ($data[img_w]) { ?>width:<?=$data[img_w]?><? } ?>">
                     			<? } else { ?>
                     				<a href="<?=$filepath?>"><?=_("파일보기")?></a>
                     			<? } ?>
                     		<? } ?>
                     	<? } else {
