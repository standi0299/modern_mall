...
case "del_coupon_member":
	...
	if ($cu || $chk) {
		msg(_("이미 회원이 사용한 쿠폰이므로 삭제할 수 없습니다."), $_SERVER[HTTP_REFERER]);
	} else {
...
case "coupon_offline_issue":
	...
	msg(_("발행이 완료되었습니다"),$_SERVER[HTTP_REFERER]);
...
case "coupon_use_update" :
	...
	$coupon_log = date("Y-m-d H:i:s") .' '._("관리자변경 처리").'<br>';
	$query = "
...
case "del_coupon_issue":
	...
	if ($cnt) {
		msg(_("이미 회원에게 발급된 쿠폰이므로 삭제할 수 없습니다."), $_SERVER[HTTP_REFERER]);
	} else {
...
case "addEvent":
   ...
   $dir_thumb = "../../data/event/$cid/$_POST[eventno]/";
   if (!is_dir($dir_thumb)) {
      mkdir($dir_thumb,0707);
      chmod($dir_thumb,0707);
   }
   
   ### 썸네일저장
   if (is_uploaded_file($_FILES[thumb_img][tmp_name])) {
      move_uploaded_file($_FILES[thumb_img][tmp_name], $dir_thumb.$_POST[eventno]);
      $size = getimagesize($dir_thumb.$_POST[eventno]);
...
case "sendmail":
	...
	if ($_POST[p]==1){	// 전체 메일링
		...
		foreach ($_POST[to] as $k=>$v){
			...
			if ($_POST[mode2]!="popup") echo "<b>[".(++$idx)."]</b> $v <span style='color:red'>["._("발송완료")."]</span></br>";
		}
	}
...
case "bottom_event_emoney":
	...
	foreach ($data as $k=>$v) {
		...
		setAddEmoney($cid, $v[mid], $_POST[emoney], " "._("댓글이벤트적립금")." [$v[no]]", $sess_admin[mid]);
	}

	if (!$_POST[is_popup]) {
		msg(_("적립금이 정상적으로 적립되었습니다."), "../promotion/event_comment.php");
	} else {
