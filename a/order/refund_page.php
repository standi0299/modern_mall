...
$order_column_arr = array("regist_date","account_flag", "account_point", "account_point", "mall_point", "m_name", "payno","","","requestdt");
...
$list = $m_order->getRefundData($cid, $addWhere.$orderby, $limit);
...
foreach ($list as $key => $value) {
   ...
   $pdata[] = "";
   $pdata[] = "<b class=\"red\"><a href=\"javascript:;\" onclick=\"popup('order_detail_popup.php?payno=$value[payno]',1200,750)\"><b>".$value[payno]."</b></a></b></td>";
   ...
   $pdata[] = substr($value[requestdt],2,14) ."(".$value[request_admin].")
              <div>". $completedt ."</div>";
   $pdata[] = "<button type=\"button\" class=\"btn btn-xs btn-danger\" onclick=\"location.href='refund_modify.php?state=$value[state]&refundno=$value[refundno]';\">"._("수정")."</button>";
