      ...
      <div class="panel-body panel-form">
         ...
         <div class="form-group">
            ...
            <div class="col-md-10 form-inline">
               ...
               <div id="vPass1"><span class="notice">[<?=_("설명")?>]</span> <?=_("등록 혹은 비밀번호 변경시에만 입력해주세요.")?></div>
            </div>
         </div>
         ...
         <div class="form-group">
            ...
            <div class="col-md-10 form-inline">
               ...
               <div id="vPass2"><span class="notice">[<?=_("설명")?>]</span> <?=_("등록 혹은 비밀번호 변경시에만 입력해주세요.")?></div>
            </div>
         </div>
         ...
         <div class="form-group">
            <label class="col-md-2 control-label"><?=_("권한")?></label>
            <div class="col-md-2">
                <select name="super" class="form-control">
                    <option value="1" <?if($data[super]==1){?>selected<?}?>><?=_("관리자")?>
                    <option value="3" <?if($data[super]==3){?>selected<?}?>><?=_("영업담당자")?>
                </select>
            </div>
         </div>
         <? } ?>
...
function chk_mid(val) {
	if (!fm.mid.value) {
		vMid.innerHTML = '<?=_("아이디를 입력해주세요.")?>';
		fm.mid.focus();
		return false;
	} else if (!_pattern(fm.mid)) {
		vMid.innerHTML = '<?=_("아이디는 영소문자,숫자,-,_ 를 사용하여 4~20자로 입력해주세요.")?>';
		fm.mid.focus();
		fm.mid.value = "";
		return false;
	}
...
function chk_password(val) {	
	if (!fm.password.value) {
		vPass1.innerHTML = '<?=_("비밀번호를 입력해주세요.")?>';
		fm.password.focus();
		return false;
	} else if (!_pattern(fm.password)) {
		vPass1.innerHTML = '<?=_("비밀번호는 영소문자,숫자,-,_ 를 사용하여 6~20자로 입력해주세요.")?>';
		fm.password.focus();
		fm.password.value = "";
		return false;
	}
	
	vPass1.innerHTML = "<span class='notice'>[" + '<?=_("설명")?>' + "]</span> " + '<?=_("등록 혹은 비밀번호 변경시에만 입력해주세요.")?>';
	
	if (fm.password.value != fm.password2.value) {
		vPass2.innerHTML = '<?=_("비밀번호 불일치")?>';
		fm.password2.focus();
		fm.password2.value = "";
		return false;
	}
	
	vPass2.innerHTML = '<?=_("비밀번호 일치")?>';
}
</script>
