...
<table>
	<thead>
		<tr>
			<th style="<?=$tableline?>"><?=_("결제일")?></th>
			<th style="<?=$tableline?>"><?=_("결제번호")?></th>
			<th style="<?=$tableline?>"><?=_("주문자")?></th>
			<th style="<?=$tableline?>"><?=_("상품명")?></th>
			<th style="<?=$tableline?>"><?=_("옵션")?></th>
			<th style="<?=$tableline?>"><?=_("수량")?></th>
			<th style="<?=$tableline?>"><?=_("금액")?></th>
		</tr>
	</thead>
	<tbody>
		<? foreach ($data as $k=>$v) {
		...
		<tr align="right" style="<?=$rowcolor?>">
			...
			<td width="200" align="left" style="<?=$tableline?>">
				<div><?=$v[opt]?></div>
				<? if ($v[addpage]) { ?>
					<div><?=_("추가페이지")?> : <?=number_format($v[addpage])?></div>
				<? } ?>
