...
foreach ($list as $key => $value) {
   ...
   ### 상태추출 (발행예정,발행중,발행마감)
   $value[state] = _("발행중");
   if ($value[coupon_issue_unlimit]) $value[state] = _("발행중");
   else if ($value[coupon_issue_sdate] && strtotime($value[coupon_issue_sdate]) > time()) $value[state] = _("발행예정");
   else if ($value[coupon_issue_edate] && strtotime($value[coupon_issue_edate])+86400 < time()) $value[state] = _("발행마감");
