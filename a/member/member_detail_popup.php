	               ...
	               <div class="table-responsive">
	                  <table class="table table-bordered table-hover">
	                     ...
	                     <? foreach ($order_list as $k => $value) {
	                     		$item_list = $m_order -> getOrdItemList($value[payno]);
								$goodsnm = "";
								$ea = "";
								
								//debug($item_list); 
								//debug(count($item_list));
								if(count($item_list) > 1) {
									foreach ($item_list as $key => $val) {
										$ea += $val[ea];
									}
									$goodsnm = $item_list[0][goodsnm] ." "._("외")." ". (count($item_list)-1) ._("건");
								}
								else {
									$goodsnm = $item_list[0][goodsnm];
									$ea = $item_list[0][ea];
								}
								
	                     ?>
	                        <tr>
	                           <td><a href="javascript:popup('../order/order_detail_popup.php?payno=<?=$value[payno]?>&mid=<?=$value[mid]?>',1100,800);"><?=$value[payno]?></a></td>
	                           <td><?=substr($value[orddt],0,10)?></td>
	                           <td><?=$goodsnm?></td>
	                           <td><?=$value[mid]?></td>
	                           <td><?=$value[orderer_name]?></td>
	                           <td><?=$ea?></td>
	                           <td><?=number_format($value[saleprice])?><?=_("원")?></td>                           
	                           <td><?=number_format($value[payprice])?><?=_("원")?></td>
	                           <td><?=$r_step[$value[paystep]]?></td>
	                        </tr>
	                     <? } ?>
	                     </tbody>
	                  </table>
