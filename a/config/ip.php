      	 ...
      	 <div class="form-group">
      	 	...
      	 	<div class="col-md-10">
      	 		<div style="font-size:18px;">
      	 			<?=_("현재접속 IP주소는")?> <b><?=$_SERVER[REMOTE_ADDR]?></b> <?=_("입니다.")?>&nbsp;&nbsp;&nbsp;
      	 			<span id="add_limited_ip"><button type="button" class="btn btn-sm btn-default"><?=_("현재 IP주소 추가하기")?></button></span>
      	 		</div>
      	 		<textarea id="limited_ip" class="form-control" name="limited_ip"><?=$cfg[limited_ip]?></textarea>
      	 		<div><span class="warning">[<?=_("주의")?>]</span> <?=_("잘못된 데이터를 입력시에는 접속에 장애가 생길수 있으니, 반드시 확인 후 입력해주세요.")?></div>
      	 		<div><span class="notice">[<?=_("설명")?>]</span> <?=_("관리자 페이지의 접속을 허용하실 IP를 엔터키로 구분하여 입력하여 주세요.")?></div>
      	 		<div><span class="notice">[<?=_("설명")?>]</span> <?=_("모든 접속을 허용시에는 값을 모두 지우시고 저장 버튼을 눌러 주세요.")?></div>
      	 	</div>
      	 </div>
