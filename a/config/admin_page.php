...
foreach ($list as $key => $value) {
   ...
   $pdata[] = $value[name];
   $pdata[] = $value[regdt];
   $pdata[] = "<a href=\"admin_write.php?mode=modify&mid=$value[mid]\"><button type=\"button\" class=\"btn btn-xs btn-primary\">"._("수정")."</button></a>";
	 
	 if ($data[super] == 0 && $sess_admin[super]) {
  		$pdata[] = "<a href=\"admin_menu_config.php?mode=menu&mid=$value[mid]\"><button type=\"button\" class=\"btn btn-xs btn-primary\">"._("메뉴설정")."</button></a>";   
	 } else {   		
   	  $pdata[] = "";
   }
