...
<div id="content" class="content">
   <!-- begin #header -->
   <form class="form-horizontal form-bordered" name="fm" method="POST" action="indb.php" onsubmit="return confirm('<?=_("수정사항은 쇼핑몰 운영에 즉시 반영됩니다.")?>' + '\r\n' + '<?=_("적용하시겠습니까?")?>')">
   ...
   <div class="panel panel-inverse"> 
      ...
      <div class="panel-body panel-form">
      	 <div class="form-group">
      	 	...
      	 	<div class="col-md-10">
      	 		<input type="radio" class="radio-inline" name="self_deliv" value="0" <?=$checked[self_deliv][0]?>> <?=_("사용안함")?>
      	 		<input type="radio" class="radio-inline" name="self_deliv" value="1" <?=$checked[self_deliv][1]?>> <?=_("사용")?>
      	 	</div>
      	 </div>
      	 ...
      	 <div class="form-group notView" id="self_deliv_div">
      	 	...
      	 	<div class="col-md-10">
      	 		...
      	 		<div id="shipping_0" class="form-inline notView"><?=_("배송비")?> <input type="text" class="form-control" name="self_shipprice" value="<?=$cfg[self_shipprice]?>" disabled type2="number"> <?=_("원")?></div>			 
				<div id="shipping_1" class="notView"><?=_("배송비가")?> <b><?=_("무료")?></b><?=_("입니다. (판매자부담)")?></div>
				<div id="shipping_3" class="form-inline notView"><?=_("주문금액이")?> <input type="text" class="form-control" name="self_shipconditional" value="<?=$cfg[self_shipconditional]?>" disabled type2="number"> <?=_("원 미만일 경우 배송비")?> <input type="text" class="form-control" name="self_shipprice" value="<?=$cfg[self_shipprice]?>" disabled type2="number"> <?=_("원")?></div>
      	 ...
      	 <div class="form-group">
            ...
            <div class="col-md-10">
            	<input type="radio" class="radio-inline" name="order_shiptype" value="0" <?=$checked[order_shiptype][0]?>> <?=_("사용안함")?>
      	 		<input type="radio" class="radio-inline" name="order_shiptype" value="1" <?=$checked[order_shiptype][1]?>> <?=_("사용")?>
            </div>
         </div>
         -->
