...
//상품관리-기타설정-제작사관리의 몰자체 제작사 정보 설정
$rid = "_self_".$cid;
   ...
   <div class="panel panel-inverse"> 
      ...
      <div class="panel-body panel-form">
      	 <div class="form-group">
      	 	...
      	 	<div class="col-md-10">
      	 		<input type="radio" class="radio-inline" name="self_release" value="0" <?=$checked[self_release][0]?>> <?=_("사용안함")?>
      	 		<input type="radio" class="radio-inline" name="self_release" value="1" <?=$checked[self_release][1]?>> <?=_("사용")?>
      	 	</div>
      	 </div>
      	 
      	 <div class="notView" id="release_div">
	      	 <div class="form-group">
	      	 	<label class="col-md-2 control-label"><?=_("자체상품 제작사")?><br><?=_("회사명")?></label>
	      	 	<div class="col-md-10 form-inline">
	      	 		<input type="text" class="form-control" name="compnm" value="<?=$data[compnm]?>" size="40" required>
	      	 		<div><span class="notice">[<?=_("설명")?>]</span> <?=_("내부 관리용 출고처명으로 사용될 명칭입니다.")?></div>
	      	 	</div>
	      	 </div>
	      	 
	      	 <div class="form-group">
	      	 	<label class="col-md-2 control-label"><?=_("자체상품 제작사")?><br><?=_("별칭")?></label>
	      	 	<div class="col-md-10 form-inline">
	      	 		<input type="text" class="form-control" name="nicknm" value="<?=$data[nicknm]?>" size="40" required>
	      	 		<div><span class="notice">[<?=_("설명")?>]</span> <?=_("장바구니와 상품상세페이지에 이용자들에게 보여질 출고처명입니다.")?></div>
	      	 	</div>
	      	 </div>
	      	 ...
	      	 <!-- 공급사에 sms발송시 사용 -->
	      	 <div class="form-group">
	      	 	<label class="col-md-2 control-label"><?=_("자체상품 제작사")?><br><?=_("연락처")?></label>
	      	 ...
	      	 <div class="form-group">
	      	 	<label class="col-md-2 control-label"><?=_("자체상품 제작사")?><br><?=_("배송비")?></label>
	      	 	<div class="col-md-10">
	      	 		<?=makeShiptypeRadioTag("shiptype", $checked[shiptype], "2")?><p>
	      	 		<div id="shipping_0" class="form-inline notView">
	      	 			<?=_("배송비")?> <input type="text" class="form-control" name="shipprice" value="<?=$data[shipprice]?>" disabled type2="number"> <?=_("원")?> 
	      	 			(<?=_("배송비원가")?> <input type="text" class="form-control" name="oshipprice" value="<?=$data[oshipprice]?>" disabled type2="number"> <?=_("원")?>)
	      	 		</div>			 
					<div id="shipping_1" class="notView"><?=_("배송비가")?> <b><?=_("무료")?></b><?=_("입니다. (판매자부담)")?></div>
					<div id="shipping_3" class="form-inline notView">
						<?=_("주문금액이")?> <input type="text" class="form-control" name="shipconditional" value="<?=$data[shipconditional]?>" disabled type2="number"> <?=_("원 미만일 경우")?> 
						<?=_("배송비")?> <input type="text" class="form-control" name="shipprice" value="<?=$data[shipprice]?>" disabled type2="number"> <?=_("원")?>
						(<?=_("배송비원가")?> <input type="text" class="form-control" name="oshipprice" value="<?=$data[oshipprice]?>" disabled type2="number"> <?=_("원")?>)
					</div>
					<div id="shipping_4" class="notView"><?=_("배송비가")?> <b><?=_("무료")?></b><?=_("입니다. (구매자부담)")?></div>  
				    <div id="shipping_9" class="notView"><?=_("배송비가")?> <b><?=_("무료")?></b><?=_("입니다.")?></div>
				    <div><span class="notice">[<?=_("설명")?>]</span> <?=_("위의 배송비는 제작사가 해당몰로 설정된 공용상품의 정산시에만 반영됩니다.")?></div>
	      	 	</div>
	      	 </div>
