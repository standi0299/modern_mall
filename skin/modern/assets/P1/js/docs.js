...
function loadPage(page, page_names) {
var page_names = page_names;
	var img = $('<img />');
	img.load(function() {
	...
	img.attr('src', page_names[page]);
}

function addPage(page, book, page_names) {
	var page_names = page_names;
	var id,
...
$("#next-button").on("click", function() {// 함수(버튼 값).핸들러?("애가 할 행동",함수(){
	$(".sample-docs").turn("next");
	//	함수("객체값").책을 넘겨야하는 행동 이름값?("행동할 이름?")
});
//});
