...
jQuery.fn.wowSlider=function(E){
    ...
    M("<div>").css({
        width:"100%",visibility:"hidden","font-size":0,"line-height":0
    }).append(a.find("li:first img:first").clone().css({
        width:"100%"
    })).prependTo(a);
    R.css({
        position:"absolute",top:0,animation:"none","-moz-animation":"none","-webkit-animation":"none"
    });
    ...
    if(E.preventCopy&&!/iPhone/.test(navigator.platform)){
        O=M('<div><a href="#" style="display:none;position:absolute;left:0;top:0;width:100%;height:100%"></a></div>').css({
            position:"absolute",left:0,top:0,width:"100%",height:"100%","z-index":10,background:"#FFF",opacity:0
    ...
    if(ak&&document.all){
        ...
        ag.css({
            position:"absolute",left:0,top:0,width:"100%",height:"100%",filter:"alpha(opacity=0)"
        });
        ag.attr({
            scrolling:"no",framespacing:0,border:0,frameBorder:"no"
        });
        ak.append(ag)
    }
    M(ak).css({
        zIndex:11,right:"5px",bottom:"2px"
    }).appendTo(S);
        ...
        if(n.length){
            ...
            if(am.length){
                ...
                function ar(aB){
                    ...
                    an.show();
                    var aC={
                        left:av.get(aB).offsetLeft-an.width()/2,"margin-top":av.get(aB).offsetTop-av.get(0).offsetTop+"px","margin-bottom":-av.get(aB).offsetTop+av.get(av.length-1).offsetTop+"px"
                    };
                    var aA=am.get(aB);
                    var az={
        ...
        M("A",T).each(function(an){
            if(an==c){
                ...
                if(!N){
                    ...
                    f.stop(true).animate({
                        left:-Math.max(Math.min(am.left,-ao.left),am.left+al.width()-T.width()),top:-Math.max(Math.min(am.top,0),am.top+al.height()-T.height())
                    })
                }
    ...
    function o(an,ar,c,ap,al){
        ...
        M(am[0]).css({
            left:(ar.left1||0)+"px",top:(ar.top1||0)+"px"
        });
        M(am[1]).css({
            left:(ar.left2||0)+"px",top:(ar.top2||0)+"px"
        });
        ...
        function ao(au){
            ...
            ,{
                duration:al,easing:(ar.easing||ap),queue:false
            })
        }
        ao(0);
        setTimeout(function(){
    ...
    function Y(aq,au){
        var at={
            position:0,top:0,left:0,bottom:0,right:0
        };
        for(var al in at){
        ...
        aq.show();
        var ap={
            width:B(aq.get(0),1,1),height:w(aq.get(0),1,1),"float":aq.css("float"),overflow:"hidden",opacity:0
        };
        for(var al in at){
        ...
        if(aq.css("position")=="static"){
            ...
            M.extend(ap,{
                position:aq.css("position"),zIndex:aq.css("z-index")
            });
            aq.css({
        ...
        ,{
            duration:au.duration,easing:au.easing
        });
        aq.animate(ao,{
            queue:false,duration:au.duration,easing:au.easing,complete:function(){
                aq.css(at);
    ...
    if(L){
        if(window.Audio&&L.canPlayType&&L.canPlayType("audio/mp3")){
            ...
            swfobject.createSWF({
                data:W+"player_mp3_js.swf",width:"1",height:"1"
            }
            ,{
                allowScriptAccess:"always",loop:true,FlashVars:"listener="+s+"&loop=1&autoplay="+(E.autoPlay?1:0)+"&mp3="+L
            }
            ,m);
            L=0
        }
