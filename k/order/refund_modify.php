...
$r_refundstate = array("접수","완료");
$r_pods_order_trans = array(-1 => "실패", 0 => "미전송", 1 => "완료");
                  ...
                  <form class="form-horizontal form-bordered" name="fm" method="POST" action="indb.php">
                     ...
                     <div class="panel-body">
                           ...
                           <table id="data-table" class="table table-striped table-bordered">
                              ...
                              <tbody>
                                 ...
                                 <tr align="center">
                                    ...
                                    <td align="right">
                                       <div>상품:<?=number_format($item[goods_price])?></div>
                                       <? if ($item[aprice]){ ?>
                                       <div>옵션:+<?=number_format($item[aprice])?></div>
                                       <? } ?>
                                       <? if ($item[addopt_aprice]){ ?>
                                       <div>추가:+<?=number_format($item[addopt_aprice])?></div>
                                       <? } ?>
                                       <? if ($item[print_aprice]){ ?>
                                       <div>인화:+<?=number_format($item[print_aprice])?></div>
                                       <? } ?>
                                       <? if ($item[dc_member]){ ?>
                                       <div>회원할인:-<?=number_format($item[dc_member])?></div>
                                       <? } ?>
                                    ...
                                    <td align="right">
                                       ...
                                       <? if ($item[dc_coupon]){ ?>
                                       <div style="margin-top:2px;" class="hand" onclick="$j(this).next().slideToggle()">쿠폰할인:-<?=number_format($item[dc_coupon])?></div><div style="position:absolute;background:#FFFFFF;width:200px;font:8pt 돋움;border:1px solid #000000;display:none;padding:2px;" align="left">사용쿠폰:<?=$item[dc_coupon_name]?></div>
                                       <? } ?>
                                    ...
                                    <div class="stxt hand" onclick="$j(this).next().slideToggle()">pod통신: <?=$r_pods_order_trans[$item[pods_trans]]?></div>
                                    <div class="stxt" style="display:none;position:absolute;border:1px solid #000000;right:0;width:100px;padding:5px;background:#FFFFFF;word-break:break-all" align="left"><?=($item[pods_trans_msg]==1)?"성공":$item[pods_trans_msg];?></div>
                                    <? } ?>
                                    </td>
                                 </tr>
                                 <? $idx++; }} ?>
                        ...
                        <table id="data-table" class="table table-striped table-bordered">
                           ...
                           <tr>
                              <th>환불총액</th>
                              <td><b class="red" id="refund_totalprice"><?=number_format($refund[cash] + $refund[pg] + $refund[emoney] + $refund[custom])?></b>원</td>
                           </tr>
                           <tr>
                              <th>관리자메모</th>
                              <td>
