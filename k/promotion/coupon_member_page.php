...
foreach ($list as $key => $value) {
	...
	if ($value[coupon_type] == "coupon_money") {
		$coupon_able_money = " - 잔액 : ".number_format($value[coupon_able_money]);
	}
	...
	if ($_GET[kind] == "off") {
		...
		foreach ($r_coupon_issue_code as $coupon_issue_code) {
			...
			if (!$value[coupon_use] && count($r_coupon_issue_code) > 1) {
				$coupon_issue_table .= "<a href=\"indb.php?mode=del_coupon_money&no=$value[no]&coupon_issue_code=$coupon_issue_code\" onclick=\"return confirm('정말로 삭제하시겠습니까?')\"><span class=\"btn btn-xs btn-danger\" style=\"margin:0;\">삭제</span></a>";
			}
	...
	if ($value[coupon_use]) {
    	$pdata[] = "사용&nbsp;<button type=\"button\" class=\"btn btn-xs btn-warning\" onclick=\"coupon_update_confirm('".$value[no]."', '0');\">"._("미사용으로전환")."</button>";
	}	else {
		$pdata[] = "미사용&nbsp;<button type=\"button\" class=\"btn btn-xs btn-info\" onclick=\"coupon_update_confirm('".$value[no]."', '1');\">"._("사용으로전환")."</button>";
	}
