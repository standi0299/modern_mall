...
<table class="table table-bordered" bgcolor="red">
   ...
   <tr>
      ...
      <td class="form-inline">
         ...
         <div style="padding-top:5px; display: none;" id="params_box" class="form-inline">
            	...
            	-<?=_("템플릿(셋) 리스트로 설정시에는")?> <b class="red"><?=_("상품의 id를 입력해주세요.")?></b> (<?=_("공란일 경우 리스트에 오류가 발생합니다.")?>)<br>
           		-<?=_("상품 id")?> :
   ...
   <tr>
      ...
      <td>
         ...
         <? } else { ?>
            <? if($data[img] && is_file("../../data/category/$cfg[skin]/$data[img]")) { ?>
               <img id="img" src="../../data/category/<?=$cfg[skin]?>/<?=$data[img]?>"/>
            <? } ?>
            <? if ($data[img] && is_file("../../data/category/m_default/$data[img]")) { ?>
               <img id="m_img" src="../../data/category/m_default/<?=$data[img]?>"/>
            <? } ?>
         <? } ?>
      </td>
   </tr>
   <tr>
      ...
      <td>
         ...
         <? } else { ?>
            <? if($data[oimg] && is_file("../../data/category/$cfg[skin]/$data[oimg]")) { ?>
               <img id="oimg" src="../../data/category/<?=$cfg[skin]?>/<?=$data[oimg]?>"/>
            <? } ?>
            <? if($data[oimg] && is_file("../../data/category/m_default/$data[oimg]")) { ?>
               <img id="m_oimg" src="../../data/category/m_default/<?=$data[oimg]?>"/>
            <? } ?>
         <? } ?>
      </td>
   </tr>
   <tr>
      ...
      <? } else { ?>
         <? if($data[left_img] && is_file("../../data/category/$cfg[skin]/$data[left_img]")) { ?>
            <img id="left_img" src="../../data/category/<?=$cfg[skin]?>/<?=$data[left_img]?>"/>
         <? } ?>
         <? if($data[left_img] && is_file("../../data/category/m_default/$data[left_img]")) { ?>
            <img id="m_left_img" src="../../data/category/m_default/<?=$data[left_img]?>"/>
         <? } ?>
      <? } ?>
      </td>
   </tr>
   <tr>
      ...
      <? } else { ?>
         <? if($data[left_oimg] && is_file("../../data/category/$cfg[skin]/$data[left_oimg]")) { ?>
            <img id="left_oimg" src="../../data/category/<?=$cfg[skin]?>/<?=$data[left_oimg]?>"/>
         <? } ?>
         <? if($data[left_oimg] && is_file("../../data/category/m_default/$data[left_oimg]")) { ?>
            <img id="m_left_oimg" src="../../data/category/m_default/<?=$data[left_oimg]?>"/>
         <? } ?>
      <? } ?>
      </td>
   </tr>
   ...
   <tr>
      ...
      <div><span class="notice">[설명]</span> 카테고리 INTRO를 사용할 경우 기본적으로 출력되는 INTRO대신 해당 카테고리에 설정한 INTRO가 출력됩니다.</div>
      </td>
   </tr>
   <tr>
      <th><?=_("모바일전용 카테고리")?></th>
   ...
   <? foreach ($r_addtion_goods_data as $r_k=>$r_v) { ?>
    ...
    <tr class="notView" id="<?=$r_k?>_div">
      ...
      <td style="padding:0;">
          ...
          <div style="margin-bottom:10px;"><span class="notice">[설명]</span> 선택된 상품 목록의 순서를 변경하시려면 드래그해주시면 됩니다.</div>
        </div>
      </td>
    </tr>
   <? }} ?>
...
function submitContents(formObj) {
   if (sendContents("header", false) && sendContents("introhtml", false)) {
      try {
...
$j(function(){
   ...
   if("<?=$data[is_url]?>"==0) {
	...
	if ($j("select[name='goods_list']").val().indexOf('_template') > -1) {
		$j('#params_box').show();
	}
	else {
		$j('#params_box').hide();
