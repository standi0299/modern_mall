...
function add_more_opt(){
   ...
   var td = document.createElement("td");
   $j(td).appendTo($j(tr));
   $j(td).html("<input type='text' class='form-control' name='podsno[]' /><a class=\"btn btn-success btn-icon btn-circle\" onclick='popup(\"http://<?=PODS10_DOMAIN?>/ProductInfo30/code_mapping.aspx?siteid=<?=$mall_info[siteid]?>&userid=<?=$mall_info[podsid]?>\",770,415)'><i class=\"fa fa-check\"></i></a>");   
   var td = document.createElement("td");
   $j(td).appendTo($j(tr));
