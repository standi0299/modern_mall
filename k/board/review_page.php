...
foreach ($list as $key => $value) {
   ...
   $pdata[] = "<div>주문번호 : $value[payno]_$value[ordno]_$value[ordseq]</div>
   ...
   else $pdata[] = "<div>$value[name]</div>(비회원)";
   $pdata[] = $r_degree[$value[degree]];
   $pdata[] = number_format($value[emoney]);
   $pdata[] = substr($value[regdt],0,10);
   $pdata[] = "<div>사용자:". $review_deny_user ."</div>
   			   <div>관리자:". $review_deny_admin ."</div>";
   $pdata[] = $review_answer;
   $pdata[] = $review_delete;
