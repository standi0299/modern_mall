...
function miniEditor(path, imagetag_flag)
{
	imagetag_flag = (typeof imagetag_flag !== undefined) ? imagetag_flag : false;
	...
	for (var i=0;i<r_textarea.length;i++){
		if (r_textarea[i].getAttribute('type')=="editor"){
			...
			miniSetFrame(r_textarea[i], imagetag_flag);
		}
	}
}

function miniSetFrame(obj, imagetag_flag)
{	
	with (obj.style){
		display = "none";
		background = "#ffffff";
	...
	toolbar.innerHTML = miniSetToolbar(imagetag_flag);
...
function miniCommand(obj,cmd,val)
{
	switch (cmd){
		...
		case "ForeColor": case "BackColor":
			...
			$('#miniColorBox').css("display", "block");
			if (cmd=="ForeColor") 
				$('#miniColorBox').css("left", "220px");
			else
				$('#miniColorBox').css("left", "240px");
...
/*** 툴바 생성 ***/
function miniSetToolbar(imagetag_flag)
{
	var return_str = "<div style='padding-left:5px;text-align:left'>" 
	+ miniSetFont() 
	+ miniSetSize() 
	+ miniSetButton("Bold") 
...
//	+ miniSetButton("CreateLink")
	...
	if (imagetag_flag)
	{
		return_str += miniSetButton("InsertImage");	
		return_str += "<img src='" + miniPath + "img/seperator.gif' align=absmiddle hspace=4>";
	}
//	+ miniSetButton("ShowBorder")
	return_str += miniTableBox()
	+ miniColorBox()
	...
	+ "<input id=_chg_mode_input type=checkbox onclick='miniChgMode(this)' onfocus=blur()> HTML"
	+ "</div>";
...
function miniSetFont()
{
	...
	var ret = "<select style='font:8pt tahoma;width:100px;' onchange=\"miniCommand(this,'FontName',this[this.selectedIndex].value)\"><option>Font";
...
{
	var ret = " <select style='font:8pt tahoma;width:100px;' onchange=\"miniCommand(this,'FontSize',this[this.selectedIndex].value)\"><option>Size";
	for (var i=1;i<=7;i++){
