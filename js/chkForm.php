...
function chkLength(field,len)
{
	...
	if (text.trim().length<len){
		alert(len + '<?echo _("자 이상 입력하셔야 합니다")?>');
		field.focus();
		return false;
	}
	return true;
...
function chkText(field,text,msg)
{
	...
	if (text==""){
		...
		var msgAct = (field.tagName!="SELECT") ? '<?echo _("입력")?>' : '<?echo _("선택")?>';
		if (!msg){
			var label = field.getAttribute("label");
			//label = _tsl(label);
			msg = label + label.josa('<?echo _("을/를")?>') + " " + msgAct + '<?echo _("해주세요")?>';
		}
		alert(msg);
...
function chkSelect(form,field,msg)
{
	...
	if (!ret){
		...
		if (!msg){
			var label = field.getAttribute("label");
			//label = _tsl(label);
			msg = label + label.josa('<?echo _("을/를")?>') + " " + '<?echo _("선택해주세요")?>';
		}
...
function chkPatten(field,patten,msg)
{
	...
	if (!patten.test(field.value)){
		...
		if (!msg){
			var label = field.getAttribute("label");
			//label = _tsl(label);
			msg = label + '<?echo _("의 입력형식이 잘못되었습니다")?>';
		}
		if (!arguments[3]){
