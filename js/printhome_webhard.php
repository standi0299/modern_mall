...
function ExecDw(editorType, self, mode, cid, order_name, upload_code) {
	...
	if (!ActiveLoader) {
		alert('<?echo _("웹하드 다운로더 설치 도중 오류가 발생했습니다. 현재 웹브라우저창을 새로고침해 주시기 바랍니다.1")?>');
		return;
	}

	if (!ActiveLoader.Update("http://podmanage.bluepod.kr/ilark/printhome/editor/")) {
		alert('<?echo _("웹하드 다운로더 설치 도중 오류가 발생했습니다. 현재 웹브라우저창을 새로고침해 주시기 바랍니다.2")?>');
		return;
	}
	...
	if (IsSuccess) {
		alert('<?echo _("다운로드가 완료되었습니다.")?>');
	} else {
		alert('<?echo _("다운로드를 취소하셨습니다.")?>');
	}
	return IsSuccess;
}
...
function ExecAxRun(fm, editorType, self, mode, cid, order_name, upload_code, mobile, flag) {
	...
	if (!ActiveLoader) {
		alert('<?echo _("Error01_웹하드 업로더 설치 도중 오류가 발생했습니다. 현재 웹브라우저창을 새로고침해 주시기 바랍니다.")?>');
		return;
	}

	if (!ActiveLoader.Update("http://podmanage.bluepod.kr/ilark/printhome/editor/")) {
		alert('<?echo _("Error02_웹하드 업로더 설치 도중 오류가 발생했습니다. 현재 웹브라우저창을 새로고침해 주시기 바랍니다.")?>');
		return;
	}
