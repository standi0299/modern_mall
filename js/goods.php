...
function refresh_price2(){
   var goodsno = $j("#goodsno").val();
	...
	$j("select[name='quick-optno[]']").each(function(index){
		...
		$j(optno[index]).append("<option value='' >선택</option>");
...
function set_price_str(ret){
   ...
   ret = ret * ea;
	$j("#goods_price").html(comma(ret)+'<?echo _("원")?>');

	//좌측 옵션보기
	$j("#quick-goods_price").html(comma(ret)+'<?echo _("원")?>');
}

function set_cprice_str(ret){
	$j("#goods_cprice").html(comma(ret)+'<?echo _("원")?>');
	var cprice = 0;
	...
	if (gap > 0){
		$j("#goods_cprice_x").html("(▼ "+comma(gap)+'<?echo _("원 할인")?>'+")");
	} else {
...
function updateOption(obj){
	...
	$j.post("indb.php", {mode:"ajax_updateOption", goodsno:goodsno, opt1:obj.options[obj.selectedIndex].title}, function(data){
		...
		for (var i=0;i<data.length;i++){
			var txt = data[i].opt2;
			if (data[i].aprice!="0") txt += " (+" + comma(data[i].aprice) + '<?echo _("원")?>'+")";
