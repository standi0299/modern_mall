	...
	function makeList(xmlStr){
		...
		$(xmlStr).find("juso").each(function(){
			...
			htmlStr += "[" + '<?echo _("도로명")?>' + "] " + roadAddress + "</a>";		
			htmlStr += "<br>[" + '<?echo _("지")?>' + "&nbsp;&nbsp&nbsp" + '<?echo _("번")?>' + "] " + $(this).find('jibunAddr').text();
	...
	function makeListMobile(xmlStr){
		...
		$(xmlStr).find("juso").each(function(){
			...
			htmlStr += "[" + '<?echo _("도로명")?>' + "] " + roadAddress + "</a>";
			htmlStr += "<span>[" + '<?echo _("지")?>' + "&nbsp;&nbsp;&nbsp;" + '<?echo _("번")?>' + "] " + $(this).find('jibunAddr').text() + "</span>";		
			htmlStr += "</li>";
		});
		htmlStr += "</ul>";
		
		if (htmlStr.indexOf('<li>') > -1) $(".postList").html(htmlStr);
		else $(".postList").html("<ul><li style=\"margin-top:80px;\">" + '<?echo _("검색 결과가 없습니다.")?>' + "</li></ul>");
	}
