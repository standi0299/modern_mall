...
function setComment(){
	$j(".comment_div").mouseenter(function(){
		...
		if (delable){
			$j(div).html("<span style='cursor: pointer;' onclick='delComment(\""+no+"\")'>" + tls("삭제") + "</span>");
		} else if (!mid){
			$j(div).html("<span style='cursor: pointer;' onclick='delCommentPassword(this,\""+no+"\")'>" + tls("삭제") + "</span>");
		}
...
function delCommentPassword(obj,no){
	...
	$j(div).html("<span id='comment_password_span'>" + tls("비밀번호를 입력해주세요.") + "</span><br><input type='password' name='password'> <input type='button' class='btn-primary' value='" + tls("확인") + "' style='cursor: pointer;' onclick='delCommentPw(this,"+no+")'>");
	$j(div).css("position","absolute");
...
function delCommentPw(obj,no){
	...
	function(data){
		if (data=="false"){
			$j("#comment_password_span").html(tls("비밀번호가 일치하지 않습니다."));
		} else if (data=="true"){
			delComment(no);
		}
	});
