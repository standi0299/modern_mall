...
//function CrossDomainMainOpenEditor(goodsno, podsno, vdp, tsid, tid){
	...
	if( $j(":input:radio[name=optno[]]:checked").length == 1  ) {
		if ( $j(":input:radio[name=optno[]_sub]:checked").length == 0  )
		{
			podsno = $j(":input:radio[name=optno[]]:checked").attr("productid");
		}
		else{
			podsno = $j(":input:radio[name=optno[]_sub]:checked").attr("productid");
		}
	}
	...
	if ($j(":input:radio[name=optno[]_sub]:checked").length) {
		optionid = $j(":input:radio[name=optno[]_sub]:checked").attr("podoptno");
	}else{
		optionid = 1;
	}
	...
	else if( $j(":input:radio[name=optno[]]:checked").length){
		//optno = $j(":input:radio[name=optno[]_sub]:checked").val();
		if ($j(":input:radio[name=optno[]_sub]:checked").length){
			optno =  $j(":input:radio[name=optno[]_sub]:checked").val();
		}
		else{
			optno = $j(":input:radio[name=optno[]]:checked").val();
		}
...
function PodsCallEditorUpdate(pods_use,podskind, podsno,goodsno,optno,storageid,productid,optionid,addopt,ea,vdp,mode,adminmode,payno,ordno,ordseq,mid,cover_id,editor_type,insert_cartno){
	// ie function 파라미터 초기화 이슈  " ')'가 필요합니다. " 211028 jtkim 220512 standi 추가
