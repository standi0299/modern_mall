  ...
  function documentOnchange(sender) {   
	...
	if (option_group_type == 'DOCUMENT') {
    	...
    	for(key in document_size) {
			...
			if(sender.options[sender.selectedIndex].text == tls("비규격")) {
				$(document_x).val("");
				$(document_y).val("");
			}    		
  ...
  function priceAction(code) {
	...
	$.get("/lib/extra_option/get_extra_option_item.php?"+param, function(data, status){
	    ...
	    if (parseInt(data) > -1)
	    {
	    	...
	    	if (price_arr)
	    	{
	          	document.getElementById("price_text_" + code).innerHTML = commify(price_arr[1]) + tls("원");	          
  ...
  function priceSum()
  {
    ...
    //추가내지가 있을 경우 값 초기화
    $("#price_text_PPP").html("0" + tls("원"));
    $("#option_supply_price_PPP").val("0");
    $("#option_price_PPP").val("0");    
    $("#price_text_OPP").html("0" + tls("원"));
	...
	if ($('#preset').val() == "100112") {
		...
		document.getElementById("price_text_C-PP").innerHTML = commify(price_sum_c) + tls("원");
   	}
    ...
    if (unitPrice > 0) {   	
    	...
    	var price_tag2 = tls("공급가") + " : " + commify(price_u) + tls("원") + " = (" + tls("원가") + " : " + commify(price_sum) + tls("원") + " - " + tls("할인") + " : " + commify(price_unit) + tls("원") + ")";
    ...
    var price_tag = "(" + tls("공급가") + " : " + commify(price_sum) + tls("원") + " + " + tls("부가세") + " : " + commify(price_tax) + tls("원") + ")";
    
    if (unitPrice > 0) {
    	price_tag += "&nbsp;&nbsp;<br/>"+price_tag2;
    }
    
    document.getElementById("select_option_sum").innerHTML = tls("결제금액") + " : " + commify(price_sum + price_tax) + " " + tls("원");
...
function getRangeSelectCnt(sender) {
	...
	if(cnt >= min && cnt <= max) {
		...
		alert("[" + tls("최소") + ":"+min+", " + tls("최대") + ":"+max+"] " + tls("사이에 수량을 입력하시기 바랍니다."));
...
function getRangeSelectCntTotal(sender) {
	...
	if (page >= min && page <= max) {
		...
		alert(tls("내지(+추가내지)/간지/면지의 전체 페이지 합이") + " [" + tls("최소") + ":" + min + ", " + tls("최대") + ":" + max + "] " + tls("사이에 수량을 입력하시기 바랍니다."));
		$(sender).val(1);
