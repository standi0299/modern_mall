...
function forwardAction() {
	...
	$j.get("/lib/extra_option/get_extra_option_item.php?"+param, function(data, status){
	    if (parseInt(data) > -1)
	    {
	    	...
	    	if (price_arr)
	    	{
	    		var price_sum = price_arr[1];
	    		
	          	//매,명,set 선택 상자가 있을 경우
			    var unitOrderCnt =  $j('#unit_order_cnt').val();
				if (unitOrderCnt > 0)
					price_sum = price_sum * unitOrderCnt;
	          	
	          	$j('#price_text').html(commify(price_sum) + tls("원"));
			    $j('#select_option_supply_price').val(price_sum);
			    $j('#select_option_sum_price').val(price_sum);
			    
	          	//$j('#price_text').html(commify(price_arr[1]) + tls("원"));
...
function makeOptionJson()
{
	...
	//페이지 수량
   	var cnt_label = $j("span[name='order_cnt_select_name']").text();
   	if(cnt_label == "") cnt_label = tls("매");	
	orderJson["est_order_option_desc"] += cnt_display_name + ":" + $j("select[name='order_cnt_select']").val() + " " + cnt_label + "<br>";
	
	//수량
   	var unit_label = $j("span[name='unit_order_cnt_name']").text();
   	if(unit_label == "") unit_label = tls("건(부)");   	
	orderJson["est_order_option_desc"] += tls("수량") + "::"+ $j("select[name='unit_order_cnt']").val() + " " + unit_label + "<br>";
