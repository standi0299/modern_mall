...
{	
	...
	if($("#order_name").val() =="") {
		alert('<?echo _("이름을 입력하세요.")?>');
		$("#order_name").focus();
		return false;
	}
	else {
		orderInfo["order_name"] = $("#order_name").val();		
	}

	orderInfo["order_cname"] = $("#order_cname").val();		
	orderInfo["order_phone"] = $("#order_phone").val();		
	
	if($("#order_mobile").val() =="") {
		alert('<?echo _("휴대전화를 입력하세요.")?>');
		$("#order_mobile").focus();
		return false;
	}
	else {
		orderInfo["order_mobile"] = $("#order_mobile").val();		
	}
	
	if($("#order_email").val() =="") {
		alert('<?echo _("이메일 주소를 입력하세요.")?>');
		$("#order_email").focus();
		return false;
	}
	else {
		orderInfo["order_email"] = $("#order_email").val();		
	}			

	//orderInfo["order_sns"] = $("#order_sns").val();
	
	if($("#agreement2").length > 0) {
		if(!$("#agreement2").is(":checked")) {
			alert('<?echo _("개인정보 취급 방침에 동의해주세요.")?>');
			$("#agreement2").focus();
			return false;
		}		
	}
	...
	$("#est_order_info").val(JSON.stringify(orderInfo));

	//initOrder(orderType, init_mode);
	
	//첨부파일 업로드 여부 확인 처리 / 16.12.23 / kdk
	if (confirm('<?echo _("파일을 첨부 하시겠습니까?")?>')) {
		//견적의뢰 관련 의뢰인정보 레이어 아웃
    	$("#dlayer-orderinfo").fadeOut();
		
		var obj = $("img[code=_sys_btn_goods_estimate_request]");
...
function initNFOrder(orderType, init_mode) 
{
	...
	if(order_type == "UPLOAD") {
		if($("input[name=est_order_type]").val() == "EDITOR") {
			$("input[name=est_order_type]").val("UPLOAD");
		}
	}
	
	//값이 없을 경우...버튼 배너화 작업으로 수정.
	if(order_type == "") order_type = $("input[name=est_order_type]").val();	
	...
	//보관함 코드 할당.
	cur_storageid = $("input[name=storageKey]").val();
...
function initJQOrder(orderType, init_mode, storageKey) 
{
	...
	$("input[name=est_order_type]").val(order_type);
...
function isValidOrder()
{
	...
	if (order_type == "UPLOAD" && order_mode == "NEW")
	{
	    ...
	    if (!result) 
	    {
	    	...
	    	if (!confirm('<?echo _("첨부된 파일이 없습니다.")?>' + "\n" + '<?echo _("파일없이 주문하시겠습니까?")?>')) {
	    		return false;
	    	}
...
function makeOptionJson()
{
	...
	orderOption["option_group_code"] = $("input[name='option_group_code']").val();	
    ...
    for (var i = 0; i < optino_group_code.length; i++) {    	
    	...
    	if (code) {
			...
			if(code == "DOCUMENT") {
      			strDocument = '<?echo _("규격")?>' + "::"+ $("select[option_group_type='DOCUMENT']").val();
      			strDocument += "(" + $("input[name='document_x']").val() + "x" + $("input[name='document_y']").val() +")mm";
        	}
        	else {
			    ...
			    for (var j = 0; j < selectOption.length; j++) {
			    	...
			    	if($(selectOption[j]).val()) {				
			    		
			    		var spl =",";
			    		var lbl = "";
			    		lbl = $("span[name='"+ $(selectOption[j]).attr("id") +"']").text();
			    		
			    		//추가내지 관련.
			    		if($("span[name='"+ $(selectOption[j]).attr("id") +"']").length > 1) {
			    			//console.log($($("span[name='"+ $(selectOption[j]).attr("id") +"']")[0]).text());
			    			lbl = $($("span[name='"+ $(selectOption[j]).attr("id") +"']")[0]).text();
			    		}			    		
			    		...
			    		if($(selectOption[j]).attr("index") == "1") spl ="|";
			    		if($(selectOption[j]).attr("index") == "2") spl ="|";
			    		if($(selectOption[j]).attr("index") == "3") spl =",";
			    		
				  		if(code == "C-PP" || code == "C-OP") {
				  			if(preset == "100112") { //책자 프리셋3(100112)
				  				if($("#chk_item_select_" + code + "_0").is(":checked")) {
				  					strCover += lbl + $(selectOption[j]).val() + spl;									
								}
				  			}
				  			else {
				  				strCover += lbl + $(selectOption[j]).val() + spl;	
				  			...
				  			strPage += lbl + $(selectOption[j]).val() + spl;
				    	}
				  		else if(code == "M-PP" || code == "M-OP") {
							if($("#chk_item_select_" + code + "_0").is(":checked")) {
				  				strMpage += lbl + $(selectOption[j]).val() + spl;									
							}
				    	}
				  		else if(code == "G-PP" || code == "G-OP") {
							if($("#chk_item_select_" + code + "_0").is(":checked")) {
				  				strGpage += lbl + $(selectOption[j]).val() + spl;									
							}				  			
				    	}
				  		else if(code.indexOf('PP-') > -1 || code.indexOf('OP-') > -1) {
				  			strAdd += lbl + $(selectOption[j]).val() + spl;
				  			
				  			var s = code.split("-");
				  			orderAddPage[s[1]] += lbl + $(selectOption[j]).val() + spl;
				    	}
				  		else if(code == "F-OP") { //책자 프리셋3(100112) 표지 필수 옵션
				  			strCover += "," + lbl + $(selectOption[j]).val() + spl;
				    	}				    	
				  		else if(code == "D-OP") { //책자 프리셋3(100112) 제본방식 JB,제본형식 HS
				  			strDocument += "," + lbl + $(selectOption[j]).val() + spl;
				    	}
				    	else {
							if($("#chk_item_select_" + code + "_0").is(":checked")) {
				  				strAfter += lbl + $(selectOption[j]).val() + spl;									
							}				    		
				    	}			    	
			    	}
			    }
      		...
      		//페이지 수량 
			var selectCntOption = $(div).find("select[name^='order_cnt_select']");
		    for (var j = 0; j < selectCntOption.length; j++) {
		    	//console.log($(selectCntOption[j]).attr("name") +":"+ $(selectCntOption[j]).val());
		    	
		    	var lbl = $("span[name='order_cnt_select_"+ $(selectCntOption[j]).attr("code") +"']").text();
		    	if($(selectCntOption[j]).attr("code").indexOf('OCNT-') > -1) { //추가내지면...
		    		lbl = $("span[name='order_cnt_select_OCNT']").text();
		    	}
	    		if(lbl !== "") lbl += ":";
	    	
		    	var label = $("span[name='"+ $(selectCntOption[j]).attr("code") +"']").text();
		    	if(label == "") label = '<?echo _("매(장)")?>';
		    	
		    	orderCnt[$(selectCntOption[j]).attr("code")] = lbl +""+ $(selectCntOption[j]).val() + ""+ label + ","; 
      		}
      	}
    }
	...
	//수량
   	var label = $("span[name='unit_order_cnt']").text();
   	if(label == "") label = '<?echo _("건(부)")?>';	
	orderJson["est_order_option_desc"] += '<?echo _("수량")?>' + "::"+ orderJson["est_order_cnt"] + " " + label + "<br>";
	...
	if(preset == "100104" || preset == "100108" || preset == "100112") { //책자 견적
		...
		if(strCover != "") orderJson["est_order_option_desc"] += '<?echo _("표지")?>' + "::["+ orderCnt["C-OCNT"] + strCover.substr(0, strCover.length -1) + "]<br>";
		if(strPage != "") orderJson["est_order_option_desc"] += '<?echo _("내지")?>' + "::["+ orderCnt["OCNT"] + strPage.substr(0, strPage.length -1) + "]<br>";
		if(strMpage != "") orderJson["est_order_option_desc"] += '<?echo _("면지")?>' + "::["+ orderCnt["M-OCNT"] + strMpage.substr(0, strMpage.length -1) + "]<br>";
		if(strGpage != "") orderJson["est_order_option_desc"] += '<?echo _("간지")?>' + "::["+ orderCnt["G-OCNT"] + strGpage.substr(0, strGpage.length -1) + "]<br>";
		
		for (key in orderAddPage) {
			if(orderAddPage[key] != "") {
				orderAddPage[key] = orderAddPage[key].replace("undefined",""); 
				orderJson["est_order_option_desc"] += '<?echo _("추가내지")?>' + "::["+ orderCnt["OCNT-"+key] + orderAddPage[key].substr(0, orderAddPage[key].length -1) + "]<br>";
			}
		}		
	}
	else if(preset == "100110") { //스튜디오 견적
		if(strPage != "") orderJson["est_order_option_desc"] += '<?echo _("옵션")?>' + "::["+ orderCnt["OCNT"] + strPage.substr(0, strPage.length -1) + "]<br>";
		if(strCover != "") orderJson["est_order_option_desc"] += '<?echo _("표지")?>' + "::["+ strCover.substr(0, strCover.length -1) + "]<br>";		
	}
	else { //낱장 견적
		if(strDocument != "") orderJson["est_order_option_desc"] += strDocument + "<br>";
		if(strPage != "") orderJson["est_order_option_desc"] += '<?echo _("옵션")?>' + "::["+ orderCnt["OCNT"] + strPage.substr(0, strPage.length -1) + "]<br>";
	...
	//후가공 옵션
	if(strAfter != "") orderJson["est_order_option_desc"] += '<?echo _("후가공")?>' + "::["+ strAfter.substr(0, strAfter.length -1) + "]";
	
	//console.log(orderJson["est_order_option_desc"]);
	//$("#txtMakeOptionJsonS2").val(JSON.stringify(orderJson));
...
function updateOptionPrice()
{
	if(ingOrder){
		ingOrder=false;
		if(befor_t_s == getNumByPrice($('#total_orgprice_obj').text()) )
			procOrder();
		else
			alert('<?echo _("가격이 다릅니다!")?>');
	}
}
...
function uploadFile(response){
	...
	if (rtn.length > 2) {
		...
		else alert('<?echo _("보관함 코드 받기 실패!!")?>' + " (" + rtn[1] + ")");	
	}
	else alert('<?echo _("보관함 코드 받기 실패!")?>' + " (" + response + ")"); 
}
//### step 5-1 파일 폼 전송 
function submitFileForm(){
	
	var upPortFrm=$("#uploadPortFrame");	
	if (!upPortFrm || upPortFrm.length<1) {
		upPortFrm = $("<iframe id='uploadPortFrame' name='uploadPortFrame' frameborder='0' width='0' height='0'/>").appendTo('body');
	}	
	setFormExternalUpload(cur_storageid);
	
	var filefrm = $('#filefrm');
	$(filefrm).attr("action",url_file_upload);
	$(filefrm).attr("target","uploadPortFrame");
	$(filefrm).submit();
}
...
function setValToHidden(name,value){
	var ctrl = $("#filefrm input[name='"+name+"']");
	if(ctrl.length>0)
	{
		$(ctrl).val(value);
	}
	...
	{
		$("#filefrm").append('<input type="hidden" name="'+name+'" value="'+value+'">');
	}
}
...
function continueFileOrder(response){
	...
	if (rtn.length > 1) {
		...
		else alert('<?echo _("파일 업로드 실패!!")?>' + "(" + rtn[1] + ")");	
	}
	else alert('<?echo _("파일 업로드 실패!")?>' + "(" + response + ")"); 
}
...
function procSubmit(sid){	
	...
	$(orderfrm).find('input[name="storageid"]').val(sid);
	...
	//console.log($(orderfrm).find('input[name="option_json"]').val());
	/*
	$('#orderfrm').ajaxSubmit({
		url: ,
		type: 'post',
...
function loopChkSubmit(response){
	...
	if(o_submitSidCnt<1)
	{
		if (o_arrErrSubmit.length > 0) {
			var errmsg='<?echo _("성공")?>' + ":" + o_arrOkSubmit.length + "/" + '<?echo _("실패")?>' + ":" + o_arrErrSubmit.length;
			for(var i=0;i<o_arrErrSubmit.length;i++){
				errmsg+='\n\n'+o_arrErrSubmit[i];
			}
			
			alert(errmsg);
			if (confirm('<?echo _("장바구니로 이동하겠습니까?")?>')) 
...
function afterHelpImgOpenLayer(imgSrc) {
	...
	//var imgWidth = 0;
	//var imgHeight = 0;	
	$("#afterHelp").attr('src',imgSrc);	
 	$("#afterHelp").each(function() {
