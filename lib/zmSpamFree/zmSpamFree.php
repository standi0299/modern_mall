...
if ( isset($_GET['zsfimg']) ) {
	...
	$thisZsfCfg = array (
		...
		'codeCfg' => array ( 1=>4, 2=>4, 3=>2, 4=>2, 5=>1, 6=>4, 7=>4, 8=>6 )
		);
	# 'SpamFree.kr' 이미지맵
	...
	if ( $thisZsfCfg['view'] == 3 ) {
		...
		$thisZsfCfg['josa'] = array(_('와'),_('과'));
		$thisZsfCfg['mi'] = array(_('는'),_('은'));
	}
		...
		switch ( $zsfN ) {
			case 1:	# A 와 B 중 큰 수는?
				...
				switch ( $thisZsfCfg['view'] ) {
					case 3:
						$zsfQ = $zsfNum[0].$thisZsfCfg['josa'][$thisZsfCfg['jong'][substr($zsfNum[0],-1)]].' '.$zsfNum[1]._('중 큰 수는?');
						break;
					default:
			...
			case 2:	# A 와 B 중 작은 수는?
				...
				switch ( $thisZsfCfg['view'] ) {
					case 3:
						$zsfQ = $zsfNum[0].$thisZsfCfg['josa'][$thisZsfCfg['jong'][substr($zsfNum[0],-1)]].' '.$zsfNum[1]._('중 작은 수는?');
						break;
					default:
			...
			case 3:	# A 더하기 B는?
				...
				switch ( $thisZsfCfg['view'] ) {
					case 1:
						...
						break;
					case 3:
						$zsfQ = $zsfNum[0].' '._("더하기").' '. $zsfNum[1].$thisZsfCfg['mi'][$thisZsfCfg['jong'][substr($zsfNum[1],-1)]].'?';
						break;
					default:
			...
			case 4:	# A 빼기 B는?
				...
				switch ( $thisZsfCfg['view'] ) {
					case 1:
						...
						break;
					case 3:
						$zsfQ = $zsfNum[0].' '._("빼기").' '. $zsfNum[1].$thisZsfCfg['mi'][$thisZsfCfg['jong'][substr($zsfNum[1],-1)]].'?';
						break;
					default:
			...
			case 5:	# A 곱하기 B는?
				...
				switch ( $thisZsfCfg['view'] ) {
					case 1:
						...
						break;
					case 3:
						$zsfQ = $zsfNum[0].' '._("곱하기").' '. $zsfNum[1].$thisZsfCfg['mi'][$thisZsfCfg['jong'][substr($zsfNum[1],-1)]].'?';
						break;
					default:
...
if ( isset($_REQUEST['zsfCode']) && trim($_REQUEST['zsfCode']) !='' ) {
   ...
   if (!$zmSpamFreeAjax || $zmSpamFreeAjax == "Y")
      echo $rslt*1;
}
unset ( $thisZsfCfg );
?>