...
class Template_
{
	...
	function changeThemeSkinFile($tplFile, $fid = 'tpl')
	{
		global $cfg, $r_skin_theme_common_tpl;
		
		//m_default 스킨은 테마를 적용하지 않는다.			20180808	chunter
		if ($cfg[skin] == "m_default") $skin_theme = "";
		else $skin_theme = $cfg['skin_theme'];
		...
		if ($skin_theme) 
		{
			...
			if ($fid == "tpl")
			{
				foreach ($r_skin_theme_common_tpl as $tplValue) 
				{
					if ($tplValue == $tplFile)
					{
						$bThemeProc = false;
			...
			if ($bThemeProc)
			{
				$result = str_replace(".htm", ".{$cfg['skin_theme']}.htm", $tplFile);
			}
		}
		return $result;
	}
	...
	{
		global $cfg, $r_skin_theme_layout, $r_skin_theme_common_tpl, $r_temple_cache_file;
		...
		if ($skin_theme) 
		{			
			foreach ($r_skin_theme_layout as $layoutValue) 
			{
				if ($fid == $layoutValue)
				{
					$bThemeProc = true;
                   ...
                   */
					...
					if ($bThemeProc)
					{
						$this->tpl_[$fid][1] = str_replace(".htm", ".{$cfg['skin_theme']}.htm", $this->tpl_[$fid][1]); 	
						//debug($this->tpl_[$fid][1]);
					}
		...
		if(!empty($r_temple_cache_file)){
			foreach ($r_temple_cache_file as $cacheKey => $cacheValue) 
			{
				if ($cacheValue[0] == $this->tpl_[$fid][1])
				{
					$this->caching = true;						
					$this->setCache($fid, $cacheValue[1], $cacheValue[0]);
					//debug($fid);
				}
			}
	...
	function & new_($obj)
	{
		...
		if (!isset($this->obj_[$class])) {
			...
			if (!class_exists($class)) include dirname(__file__).'/tpl_plugin/object.'.$obj.'.php';
			$n = func_num_args();
			if ($n>1) {
				for ($i=1; $i<$n; $i++) {
