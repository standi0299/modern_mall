...
class Template_Compiler_
{
	...
	function _close_loop()
	{
		...
		// 1: key_, 2: value_, 4: index_, 8: size_, 16: last_
		$_key = $info['foreach_bit']&1 ? '$TPL_K'.$depth.'=>' : '';
		if ($info['foreach_bit']&4 || $info['foreach_bit']&16) {
			$_idx1='$TPL_I'.$depth.'=-1;';
			$_idx2='$TPL_I'.$depth.'++;';
		} else {
			$_idx1='';
		...
		$_last1 = ($info['foreach_bit']&16) ? '$TPL_L'.$depth.'=($TPL_S'.$depth.'-$TPL_I'.$depth.'==1);' : '';
		...
		if ($loop_id==='*') {
			$_size = ($info['foreach_bit']&8 || $info['foreach_bit']&16) ? '$TPL_S'.$depth.'=count($TPL_R'.$depth.');' : '';
			$split.= $_size.$_idx1.'foreach($TPL_R'.$depth.' as '.$_key.'$TPL_V'.$depth.'){'.$_idx2.$_last1.$_sub_loop_size.'?>';
		} else {
	...
	function _compile_array($subject, $end='')
	{
		...
		case 'last_':
			if ($end!=='stop') $this->exp_error[]=array(-1,$subject);
			elseif (isset($this->exp_loopvar[$depth])) $this->exp_loopvar[$depth] |= 16;
			else $this->exp_loopvar[$depth] = 16;
			return '$TPL_L'.$depth;
		default :
