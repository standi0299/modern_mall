...
*/
	...
	$result = getOptionSizePr($print_goods_item[$goods_no]);
    //debug($result);
    ...
    if ($extra_use[extra_print_pr_price_type] == "MM") { //1mm당 단가
        $result = getOptionPaperPr1mm($print_goods_item[$goods_no]);    
    }
    else {
        $result = getOptionPaperPr($print_goods_item[$goods_no]);
    }
    ...
    */
    ...
    foreach ($print_goods_item[$goods_no] as $OptionKey => $OptionValue) {
        ...
        if ($OptionKey == "directupload_use" || $OptionKey == "cnt_use" || $OptionKey == "vat_use")
        {
            $result = $OptionValue;
        }
        else 
