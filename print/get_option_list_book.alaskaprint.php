<?
    ...
    $result = makeOptionInputTag_Print1(getOptionPrint($print_goods_item[$goods_no], "outside_print"), $_POST['outside_print1'], $_POST['outside_print3_check'], "outside", "0", "select", $funcName);
    $response[outside_print1] = $result;

    //사용자 화면 양면단면 옵션 태그 만들기
    $result = makeOptionInputTag_Print2(getOptionPrint($print_goods_item[$goods_no], "outside_print"), $_POST['outside_print2'], "outside", "0", "select");
    $response[outside_print2] = $result;
    
    //사용자 화면 별색추가 옵션 태그 만들기
    $result = makeOptionCheckTag_Print3(getOptionPrint($print_goods_item[$goods_no], "outside_print"), $_POST['outside_print3'], "outside", "0", "select");
    $response[outside_print3] = $result;

    //알래스카 별색 사용여부 처리.
    if ($response[outside_print3] == "<option value=''>선택해주세요.</option>") $response[outside_print3_use] = "N";
    ...
    if ($goods_type == "OS02") {
        ...
        $result = makeOptionInputTag_Print4(getOptionPrint($print_goods_item[$goods_no], "outside_print_back"), $_POST['outside_print4'], $_POST['outside_print6_check'], "outside", "1", "select", $funcName);
        $response[outside_print4] = $result;
    
        //옵셋 책자 별색추가 옵션 태그 만들기
        $result = makeOptionCheckTag_Print6(getOptionPrint($print_goods_item[$goods_no], "outside_print_back"), $_POST['outside_print6'], "outside", "0", "select");
    ...
    if (is_array($_POST[inside_paper_group])) {
        ...
        foreach ($_POST as $ItemKey => $ItemValue) 
        {
            if (strpos($ItemKey, "inside_print1_") !== false)
            {
                ...
                $result = makeOptionInputTag_Print1(getOptionPrint($print_goods_item[$goods_no], "inside_print"), $_POST[$ItemKey], $_POST["inside_print3_check_".$postArr[2]], "inside", $postArr[2], "select");
                $response[$ItemKey] = $result;
            }
        }
    }
    else {
        $result = makeOptionInputTag_Print1(getOptionPrint($print_goods_item[$goods_no], "inside_print"), "", "", "inside", "0", "select");
    ...
    if (is_array($_POST[inside_paper_group])) {
        ...
        foreach ($_POST as $ItemKey => $ItemValue) 
        {
            ...
            {
                $postArr = split('_', $ItemKey);
                $result = makeOptionInputTag_Print2(getOptionPrint($print_goods_item[$goods_no], "inside_print"), $_POST[$ItemKey], "inside", $postArr[2], "select");
                $response[$ItemKey] = $result;
            }
        }        
    }
    else {
        $result = makeOptionInputTag_Print2(getOptionPrint($print_goods_item[$goods_no], "inside_print"), "", "inside", "0", "select");
    ...
    if (is_array($_POST[inside_paper_group])) {
        ...
        foreach ($_POST as $ItemKey => $ItemValue) 
        {
            ...
            {
                $postArr = split('_', $ItemKey);
                $result = makeOptionCheckTag_Print3(getOptionPrint($print_goods_item[$goods_no], "inside_print"), $_POST[$ItemKey], "inside", $postArr[2], "select");
                $response[$ItemKey] = $result;
            }
        }
    }
    else {
        $result = makeOptionCheckTag_Print3(getOptionPrint($print_goods_item[$goods_no], "inside_print"), "", "inside", "0", "select");
        //$response[inside_print3][] = $result;
        $response[inside_print3_0] = $result;
    }

    //알래스카 별색 사용여부 처리.
    if ($response[inside_print3_0] == "<option value=''>선택해주세요.</option>") $response[inside_print3_0_use] = "N";    
    ...
    if (is_array($_POST[inpage_paper_group])) {
        ...
        foreach ($_POST as $ItemKey => $ItemValue) 
        {
            ...
            {
                $postArr = split('_', $ItemKey);
                $result = makeOptionInputTag_Print1(getOptionPrint($print_goods_item[$goods_no], "inpage_print"), $_POST[$ItemKey], $_POST["inpage_print3_check_".$postArr[2]], "inpage", $postArr[2], "select");
                $response[$ItemKey] = $result;
            }
        }
    }
    else {
        $result = makeOptionInputTag_Print1(getOptionPrint($print_goods_item[$goods_no], "inpage_print"), "", "", "inpage", "0", "select");
    ...
    if (is_array($_POST[inpage_paper_group])) {
        ...
        foreach ($_POST as $ItemKey => $ItemValue) 
        {
            ...
            {
                $postArr = split('_', $ItemKey);
                $result = makeOptionInputTag_Print2(getOptionPrint($print_goods_item[$goods_no], "inpage_print"), $_POST[$ItemKey], "inpage", $postArr[2], "select");
                $response[$ItemKey] = $result;
            }
        }        
    }
    else {
        $result = makeOptionInputTag_Print2(getOptionPrint($print_goods_item[$goods_no], "inpage_print"), "", "inpage", "0", "select");
    ...
    if (is_array($_POST[inpage_paper_group])) {
        ...
        foreach ($_POST as $ItemKey => $ItemValue) 
        {
            ...
            {
                $postArr = split('_', $ItemKey);
                $result = makeOptionCheckTag_Print3(getOptionPrint($print_goods_item[$goods_no], "inpage_print"), $_POST[$ItemKey], "inpage", $postArr[2], "select");
                $response[$ItemKey] = $result;
            }
        }
    }
    else {
        $result = makeOptionCheckTag_Print3(getOptionPrint($print_goods_item[$goods_no], "inpage_print"), "", "inpage", "0", "select");
        //$response[inpage_print3][] = $result;
        $response[inpage_print3_0] = $result;
    }

    //알래스카 별색 사용여부 처리.
    if ($response[inpage_print3_0] == "<option value=''>선택해주세요.</option>") $response[inpage_print3_0_use] = "N";
    
    

    $outside_gloss = getOptionData($print_goods_item[$goods_no]['outside_gloss'], "outside_gloss");
    //$response[outside_gloss] = makeOptionInputTag($outside_gloss, "outside_gloss", $_POST['outside_gloss'], "outside_gloss", "outside_gloss", "", "select");
    $response[outside_gloss] = makeOptionSelectTag($outside_gloss, $_POST['outside_gloss']);
    ...
    foreach ($print_goods_item[$goods_no] as $OptionKey => $OptionValue) {
        ...
        else 
        {
            ...
            /*if ($OptionKey == "instant" || $OptionKey == "wing")
                ...
                $result = makeOptionInputTag($result, $OptionKey, $_POST['opt_'.$OptionKey], $OptionKey, "opt_".$OptionKey, "", "select");*/
