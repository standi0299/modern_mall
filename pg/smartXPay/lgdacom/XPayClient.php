...
class XPayClient
{
	...
	function XPayClient($home_dir, $mode="real")
	{
		...
		//log 경로 변경			20160108	chunter
		$this->config["log_dir"] = dirname(__FILE__) ."/../log/";
		//lg PG 키 할당			20160108	chunter
		$this->config[$cfg[pg][lgd_mid]] = $cfg[pg][lgd_mertkey];
		$test_lgd_mid = 't'.$cfg[pg][lgd_mid];
		$this->config[$test_lgd_mid] = $cfg[pg][lgd_mertkey];
		//debug($this->config);
		//exit;
		...
		if (!file_exists($this->log_file)) {
			$handle = fopen($this->log_file, 'w') or die('Cannot open file:  '.$my_file);
			fclose($handle);
		}
