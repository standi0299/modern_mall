<?php
    ...
    ### 로그저장						
		writeLog($LGD_OID, $HTTP_POST_VARS);
    ...
    if ( $LGD_HASHDATA2 == $LGD_HASHDATA ) { //해쉬값 검증이 성공이면
        if ( "0000" == $LGD_RESPCODE ){ //결제가 성공이면
        	...
        	if( "R" == $LGD_CASFLAG ) {
        		$bankinfo	= "$LGD_FINANCENAME / $LGD_ACCOUNTNUM / $LGD_CASTAMOUNT";
						//$add_fields	.= ",bankinfo	= '$bankinfo'";
	
						$pglog .= "결제방법 : 가상계좌,	가상계좌 : ".$bankinfo;					               
            $step = 1;
             ...
             */  
						...
						$step = -1;
						$pglog = "거래번호 : $pgcode /	[" .$LGD_RESPCODE. "] ".$LGD_RESPMSG;
