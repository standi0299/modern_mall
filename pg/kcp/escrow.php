...
include "../../conf/config.php";
    ...
    {
		if( $res_cd == "0000" ){
			$db->query("update mini_order set escrowok=1 where ordno='$_POST[ordno]'");
			echo "ok";
		} else {
			
			list ($escrowok) = $db->fetch("select escrowok from mini_order where ordno='$_POST[ordno]'",1);
			
			/*
			if ($escrowok) echo "<script>alert('이미 정상처리된 주문건입니다');</script>";
			else echo "<script>alert('[$res_cd] $res_msg');</script>";
			*/
			if ($escrowok) echo "duplicate";
			else echo "[$res_cd] $res_msg";
