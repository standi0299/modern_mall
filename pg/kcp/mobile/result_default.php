...
<?
  ...
  if ( $req_tx == "pay" )
  {
    ...
    if ( $bSucc == "false" )
    {
    	if ( $res_cd == "0000" )
      {
      	$res_msg_bsucc = "결제는 정상적으로 이루어졌지만 쇼핑몰에서 결제 결과를 처리하는 중 오류가 발생하여 시스템에서 자동으로 취소 요청을 하였습니다.<br />쇼핑몰로 전화하여 확인하시기 바랍니다." ;
				$pg_log = "결제는 정상적으로 이루어졌지만 쇼핑몰에서 결제 결과를 처리하는 중 오류가 발생하여 시스템에서 자동으로 취소 요청을 하였습니다.<br />쇼핑몰로 전화하여 확인하시기 바랍니다." ;
     	}
      else
     	{
      	$res_msg_bsucc = "결제는 정상적으로 이루어졌지만 쇼핑몰에서 결제 결과를 처리하는 중 오류가 발생하여 시스템에서 자동으로 취소 요청을 하였으나<br /><b>취소가 실패 되었습니다.</b><br />쇼핑몰로 전화하여 확인하시기 바랍니다.";
				$pg_log = "결제는 정상적으로 이루어졌지만 쇼핑몰에서 결제 결과를 처리하는 중 오류가 발생하여 시스템에서 자동으로 취소 요청을 하였으나<br /><b>취소가 실패 되었습니다.</b><br />쇼핑몰로 전화하여 확인하시기 바랍니다." ;
      }
  	}
  }
	else
	{
		...
		$pg_log = "주문이 취소 되었습니다.<br />다시 주문해주시기 바랍니다." ;
	}
  ...
  if ( $req_tx == "pay" )
  {
    ...
    if ( $bSucc != "false" )
    {
      if ( $res_cd == "0000" )
      {
      	//go("/order/payend.php?payno=$ordr_idxx");
				$page_return_url = "/order/payend.php?payno=$ordr_idxx";
				echo "<script>parent.location.href='".$page_return_url."';</script>";
				exit;
      }
			else
			{
				$pg_log = "결제에 실패하였습니다. [오류코드 : $res_cd / $res_msg]";
			}
   	}
    else
		{
	...
	//$res_cd "3001"	-> 사용자 취소			20160923		chunter
	if ($ordr_idxx && $res_cd != "3001")
	{
		//결제 실패 처리...				20160921		chunter
		$step = -1;
		$payno = $ordr_idxx;     
		$pglog = $pg_log . " / 거래번호 : " .$tno;			
		include "../../pg_pay_fail.php";
			
		//go("/order/payfail.php?payno=$ordr_idxx");		
		$page_return_url = "/order/payfail.php?payno=$ordr_idxx";
	} else {
		//사용자 주문 취소인 경우
		$beforeScript = "alert('취소하셨습니다');";
		$page_return_url = "/order/cart.php";
	}
	echo "<script>$beforeScript parent.location.href='".$page_return_url."';</script>";
