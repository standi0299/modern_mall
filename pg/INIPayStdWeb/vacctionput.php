...
if( $PG_IP == "203.238.37" || $PG_IP == "210.98.138" || $PG_IP == "112.216.14" || $PG_IP == "39.115.212" || $PG_IP == "183.109.71"){	//PG에서 보냈는지 IP로 체크
	...
	$logfile = fopen("./log/".$no_oid, "a+" );

	fwrite( $logfile,"************************************************");
	fwrite( $logfile,"ID_MERCHANT : ".$id_merchant."\r\n");
	fwrite( $logfile,"REQUEST TIME : ".date("Y-m-d G:i:s")."\r\n");
  ...
  {	 
    ...
    //결제 단계가 1이 아니면 PODS통신 및 결제, 아이템 스텝 변경 불가, 자동 메일, sms 발송 불가 / 15.04.30 / kjm
  ...
  { 
	...
	if ($ret){
		...
		$step		= 2;
		$orderInputRequest = false;
		$vbankInputFlag = true;				//가상계좌 입금처리
		$payno = $no_oid;
		$pgcode		= $no_tid;
		$namePayer	= $nm_input;
		$escrow		= 0;					
		$PG_Account = $amt_input;
		//$pglog = "결제방법 : inpaystd / 승인번호 : " .$pgcode. "(" .date('Y:m:d H:i:s'). ") / 금액 : " .$PG_Account." / 결과코드 : " .$type_msg;
