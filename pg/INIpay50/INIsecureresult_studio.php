...
$pgPayKind = $_POST[pg_pay_kind];
if (!$pgPayKind)
    $pgPayKind = "fix_member";
//정액 회원
$pgPayPrice = $inipay -> GetResult('TotPrice');
//결제 금액
...
switch ($pgPayKind) {
    ...
    case 'credit' :
        ...
        $gourl = "/member/payment_result.php";
        ### 중복결제체크
        $query = "select * from tb_deposit_payno where payno='$payno'";
        break;
}

$gourl = "/mypage/c2_deposit_pay.php";
$flag = "ok";
//debug($query)   ;
### 중복결제체크
$data = $db -> fetch($query);
...
if (!$data[payno] && !$data[paydt]) {
    ...
    if (!strcmp($inipay -> GetResult('ResultCode'), "00")) {### 카드결제 성공
        ...
        switch ($pgPayKind) {
            ...
            case 'credit' :
              //lib.cust.order.func.php 로 함수 생성     20150513    chunter
              depositAccountProc($payno, $pgPayPrice, $pglog);
              break;
        }
...
<script>
function goResult(url, flag){
    if(flag == "ok"){
        alert("예치금 충전이 완료되었습니다.");
    }
    location.replace(url);
}
goResult("<?=$gourl?>", "<?=$flag?>");</script>