...
$r_stat_msg       = $_POST[ "stat_msg"       ];  // 변경에스크로상태메세지
	...
	if ( $r_res_cd == "0000" )
	{
		...
		ob_start();
		$fp = fopen("./log/$r_order_no"."_VBANK","w");
		debug($_REQUEST);
		$ret = ob_get_contents();
		fwrite($fp,$ret);
		fclose($fp);
		...
		{		
			...
			$payno = $r_order_no;
			$pgcode		= $r_cno;
			$namePayer	= $r_deposit_nm;
			$escrow		= ($r_escrow_yn=="Y") ? 1 : 0;
			$pglog		= "$payno (".date('Y:m:d H:i:s').")거래번호 : ".$pgcode."승인금액 : ".$r_amount;
			$PG_Account = $r_amount;
