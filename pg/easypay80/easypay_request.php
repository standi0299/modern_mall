<?php
    ...
    $g_home_dir   = dirname(__FILE__);
    $g_cert_file  = dirname(__FILE__)."/cert/pg_cert.pem";
    $g_log_dir    = dirname(__FILE__)."/log";
    $g_log_level  = "99";
    ...
    $page_return_url = "/order/payfail.php?payno=".$order_no;
    /* -------------------------------------------------------------------------- */
    /* ::: 가맹점 DB 처리                                                         */
    /* -------------------------------------------------------------------------- */
    ...
    if ( $res_cd == "0000" ) {
    	//$bDBProc = "true";     // DB처리 성공 시 "true", 실패 시 "false"
     	...
     	$step		= 2;
			...
			$pglog		= "$payno (".date('Y:m:d H:i:s').") / 거래번호 : ".$pgcode." / 승인금액 : ".$PG_Account;
...
//81:배치인증
			...
			{
				case "11": //신용카드
					$pglog .= " / 결제방법 : 카드결제 - 승인번호 : " .$pgcode. ", 카드회사 : " .$r_issuer_nm;
					break;
				case "21": //계좌이체		
					$pglog .= " / 결제방법 : 계좌이체 - ".$r_bank_nm;
					break;		
				case "22": //가상계좌(무통장)
					$r_bank_nm = iconv("euc-kr", "utf-8", $r_bank_nm);
					$bankinfo	= "{$r_bank_nm} / {$r_account_no} / {$PG_Account}";						
					//$bankinfo	= " / / ";
			...
			{
				$bDBProc = true; //DB처리 실패시 false로 변경해 주세요.
        ...
        {          
					...
					//DB 처리  실패시 exm_pay도 실패 처리한다.
					$payno = $order_no;
					$step = -1;
					$pglog = "RES_MSG - DB 처리 실패로 망취소, RES_CD - $res_cd";
		...
		$pglog = "RES_CD - {$res_cd}, RES_MSG - {$res_msg}";
		include "../pg_pay_fail.php";
		//debug("update exm_pay set escrow	= '$escrow', pglog	= '$pglog', paystep	= '$step' where payno	='$payno' and paydt is null");
	}
