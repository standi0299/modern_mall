...
$account_type = $_POST["account_type"];//결제 요청한 상품.
$goods_type = $_POST["goods_type"];//결제 상품 정보(금액)
...
if ($req_tx == "pay") {
   if ($res_cd == "0000") {
      ...
      $bSucc = "true";
      //DB 처리
      //포인트 결제    //가상 결제는 처리하지 않음.
      if ($bDBProc) {
         //include_once "./inc_pay_success_proc.php";

         $data = $db -> fetch("select * from tb_pretty_account_payno where payno='$ordr_idxx'");
         if ($data && $data[payno]) {
            //결제 요청금액과 실제 결제 금액이 같은 경우만 처리한다.       20150710    chunter
            if ($amount == $data[account_money]) {
               $db -> query("update tb_pretty_account_payno set paydt=now() where payno='$ordr_idxx'");

               $pglog = "주문번호 : $ordr_idxx, PG결제번호 : $tno, 결제금액 : $amount";
               if ($_POST[account_type] == "P") {
                  setPointProcess($cid, $_POST[account_point], "21", '', '', $pglog, $ordr_idxx, $tno, $account_pay_method, $amount);
                  //포인트 처리
               }
               //스토리지 결제
               else if ($_POST[account_type] == "S") {
                  updateStorageData($cid, $_POST[storage_size], $_POST[storage_month], $_POST[storage_account_kind], $amount, $tno, $ordr_idxx, $account_pay_method, $pglog, $_POST[account_detail_info]);
               }
               //printgroup 서비스 결제
               else if ($_POST[account_type] == "E") {
                  updateServiceData($cid, $_POST[storage_month], $_POST[storage_account_kind], $amount, $tno, $ordr_idxx, $account_pay_method, $pglog, $_POST[account_detail_info]);
               } else {
                  $bSucc = "false";
               }
            } else {
...
if ($req_tx == "pay") {
   if ($res_cd == "0000") {
      if ($bSucc == "false") {
         ...
         $c_PayPlus -> mf_do_tx($trace_no, $g_conf_home_dir, $g_conf_site_cd, $g_conf_site_key, $tran_cd, "", $g_conf_gw_url, $g_conf_gw_port, "payplus_cli_slib", $ordr_idxx, $cust_ip, $g_conf_log_level, 0, 0, $g_conf_log_path);         // 응답 전문 처리
